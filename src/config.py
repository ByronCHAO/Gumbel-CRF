class Config:
  ...
  def __init__(self):
    ...
    # 'autodecoder'
    # 'gaussian_vae'
    self.model_name = 'latent_temp_crf_ar'
    ...
    self.data_path = {
        'e2e_toy': {
            'train': '../data/e2e-dataset/toyset.csv',
            'dev': '../data/e2e-dataset/toyset.csv',
            'test': '../data/e2e-dataset/toyset.csv',
    ...
    self.max_sent_len = {'e2e_toy': 37, 'e2e': 37, 'ptb': 39, 'mscoco': 16}
    # 95% + 1, count _GOO in source and _EOS in target
    self.max_dec_len = {'e2e_toy': 38, 'e2e': 38, 'ptb': 40, 'mscoco': 16}
    self.max_bow_len = {'e2e_toy': 37, 'e2e': 37, 'ptb': 39, 'mscoco': 16}

...
from .semirings import (CrossEntropySemiring, EntropySemiring, KLDivergenceSemiring, KMaxSemiring, LogSemiring,
                        MaxSemiring, MultiSampledSemiring, StdSemiring, GumbelCRFSemiring)
...
class StructDistribution(Distribution):
    ...
    * First-and second-order expectation semirings with applications to minimum-risk training on translation forests :cite:`li2009first`
    ...
    def gumbel_crf_st(self, temperature=1.0):
        with torch.enable_grad():
            st_gumbel = self._struct(GumbelCRFSemiring(temperature, True)).marginals(
    ...
    def gumbel_crf(self, temperature=1.0):
        with torch.enable_grad():
            gumbel = self._struct(GumbelCRFSemiring(temperature, False)).marginals(
                self.log_potentials, self.lengths
            )
            return gumbel
...
class DependencyCRF2O(StructDistribution):
    def __init__(self, log_potentials, lengths=None, args={}, multiroot=True):
        super(DependencyCRF2O, self).__init__(self.struct.convert(*log_potentials), lengths, args)
        setattr(self.struct, "multiroot", multiroot)
